...
local function OnEvent(self)
	...
	-- Custom format: "100 dura" instead of "Durability: 100%"
	self.text:SetFormattedText('%s%d|r dura', hex, totalDurability)

	if totalDurability <= (db and db.percThreshold or 35) then
...
local function OnEnter()
	...
	if totalRepairCost > 0 then
		DT.tooltip:AddLine(' ')
		DT.tooltip:AddDoubleLine(REPAIR_COST, E:FormatMoney(totalRepairCost, (db and db.goldFormat) or 'BLIZZARD', not (db and db.goldCoins)), .6, .8, 1, 1, 1, 1)
	end
...
-- Register the Durability datatext
DT:RegisterDatatext('NE:Dura', nil, {'UPDATE_INVENTORY_DURABILITY', 'MERCHANT_SHOW'}, OnEvent, nil, Click, OnEnter, nil, "NE: Dura", nil, ApplySettings)
